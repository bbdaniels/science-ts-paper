...
use "/Users/bbdaniels/Dropbox/WorldBank/Energy/household_microgeneration_project/data and analysis/NSPL_NOV_2013_UK.dta", clear
...
* Time Series
	...
	use "$datadir/Data/Public/installs_lsoas.dta",clear
		...
		collapse (rawsum) installed_capacity n_installs (mean) period, by(llsoa_code date) fast

			tsset llsoa_code date
         drop if date == .
			tsfill , full
        infill period , by(date)
  ...
  // Constructed data
    rename installed_capacity capacity

    gen income = tariffpkwh * 8.5 * capacity
